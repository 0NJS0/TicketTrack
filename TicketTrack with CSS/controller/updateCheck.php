...
if (isset($_REQUEST['update'])) {
    ...
    if (empty($errors)) {
        if (checkIfValuesExist($usernameconfirm,$full_name_confirm, $phone_confirm, $current_username)) {
            $errors[] = "Credentials Already belongs to another user";
        } else {
            
                $status = updateUser($usernameconfirm, $full_name_confirm, $phone_confirm, $dob,$current_username);
                if ($status) {
